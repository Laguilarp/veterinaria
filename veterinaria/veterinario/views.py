...
@login_required
#@validador
def listar_veterinarios(request,search=None):
    try:
        parametros = ''
        veterinarios = Veterinario.objects.filter(status=True)
        if 'search' in request.GET:
            ...
            ss = search_.split(' ')
            if len(ss) == 1:
                veterinarios = veterinarios.filter(Q(persona__nombres__icontains=search) |
                                           Q(persona__apellido1__icontains=search) |
                                           Q(persona__apellido2__icontains=search) |
                                           Q(persona__documento__icontains=search))
            else:
                veterinarios = veterinarios.filter((Q(persona__apellido1__icontains=ss[0]) & Q(persona__apellido2__icontains=ss[1])) |
                                           (Q(persona__nombres__icontains=ss[0]) & Q(persona__nombres__icontains=ss[1])))

        paginator = Paginator(veterinarios, 25)
        ...
        context = {
            ...
            'page_titulo': "Veterinarios",
            'titulo': "Veterinarios",
            'search': search,
            'parametros': parametros,
        }
        return render(request, 'veterinario/inicio.html', context)
